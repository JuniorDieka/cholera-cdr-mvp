"""Unit tests for ML forecasting module using mock Prophet."""
import pytest
import pandas as pd
import numpy as np
...
def test_mock_prophet_model_training():
    """Test mock Prophet model training with sample data."""
    # Create sample data
...
def test_mock_forecast_explainability():
    """Test forecast explainability features with mock Prophet."""
    # Create sample data
    ...
    # Train model
    model, forecast, _ = mock_train_prophet_model(df, forecast_weeks=4)
    ...
    assert explanation['forecast_weeks'] == 30  # 26 historical + 4 forecast


def test_mock_model_performance_evaluation():
...
def test_mock_forecast_summary():
    """Test forecast summary generation with mock Prophet."""
...
def test_mock_prophet_model_with_insufficient_data():
    """Test mock Prophet model behavior with minimal data."""
    ...
    # Should still work
    model, forecast, metrics = mock_train_prophet_model(df, forecast_weeks=2)
    
    assert model is not None
    assert len(forecast) == 6  # 4 original + 2 forecast


def test_mock_forecast_with_zero_cases():
    """Test mock forecasting with zero cases."""
...
def test_mock_model_reproducibility():
    """Test that mock model training is reproducible."""
    # Create sample data
    dates = pd.date_range(start='2024-01-01', periods=20, freq='W')
    cases = [100 + i*2 for i in range(20)]
    ...
    # Train model twice
    model1, forecast1, metrics1 = mock_train_prophet_model(df, forecast_weeks=2)
    model2, forecast2, metrics2 = mock_train_prophet_model(df, forecast_weeks=2)
    
    # Results should be similar (mock is deterministic)
